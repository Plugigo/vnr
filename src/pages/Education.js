...
const TYPES = [
  { key: 'info', label: 'School/College Info' },
  { key: 'exam', label: 'Exam Schedules/Results' },
  { key: 'resource', label: 'Learning Resources' }
];
...
export default function Education() {
  ...
  useEffect(() => {
    setLoading(true);
    const q = query(collection(db, 'education'), orderBy('type'));
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      await updateDoc(doc(db, 'education', editId), {
        type: editType,
        title: editTitle,
        desc: editDesc,
        link: editLink
                ...
                <div key={item.id} style={{background:'#fff',marginBottom:'1rem',padding:'1rem',borderRadius:'8px',boxShadow:'0 1px 4px rgba(0,0,0,0.05)'}}>
                  {isAdmin && editId === item.id ? (
                    ...
                    <>
                      ...
                      {item.link && <div><a href={item.link} target="_blank" rel="noopener noreferrer">Resource Link</a></div>}
